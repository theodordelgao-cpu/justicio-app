...
def index():
    ...
    return STYLE + f"<h1>⚖️ JUSTICIO</h1><p>Bonjour <b>{session.get('name')}</b></p><a href='/scan' class='btn'>🔍 ANALYSER MES LITIGES</a><br><a href='/logout' class='btn-logout'>Se déconnecter</a>" + FOOTER
...
def pre_payment():
    ...
    subject = request.args.get("subject", "Référence inconnue")
    
    new_litigation = Litigation(user_email=session.get('email'), company=company, amount=amount, law=law, subject=subject, status="Attente Paiement")
    db.session.add(new_litigation)
    db.session.commit()
    
    return STYLE + f"""<div style='text-align:center;'><h1>Validation</h1><p>Dossier {company} : <b>{amount}</b>.</p><div class='card' style='border-left-color:#10b981;'><h3>⚖️ Procédure Juridique</h3><p>En cliquant ci-dessous, vous activez l'envoi d'une <b>mise en demeure formelle</b> au siège de {company}.</p></div><a href='/setup-payment' class='btn' style='background:#10b981;'>ACTIVER LA PROCÉDURE</a></div>""" + FOOTER
...
def success_page():
    ...
    <div style='text-align:center; padding-top:50px;'>
        ...
        <h1>Mise en demeure envoyée !</h1>
        <p>Votre dossier juridique a été transmis avec succès.</p>
        <div class='card' style='border-left-color:#10b981;'>
            <h3>📅 Prochaines étapes</h3>
            <ul style='text-align:left; color:#475569;'>
                <li>Le service juridique de la compagnie a reçu notre mise en demeure.</li>
                <li>Ils ont légalement <b>8 jours</b> pour répondre.</li>
                <li>Nous vous notifierons dès réception de la proposition de remboursement.</li>
            </ul>
        </div>
        <a href='/' class='btn'>Retour au tableau de bord</a>
    </div>
    """ + FOOTER
...
À l'attention du Service Juridique / Réclamations de {litigation.company},

Je soussigné(e), {user.name}, agissant via la plateforme de protection juridique Justicio, vous notifie par la présente une mise en demeure de payer.

EXPOSÉ DU LITIGE :
Il apparaît qu'un incident relevant de votre responsabilité (Retard/Annulation/Non-conformité) a été constaté concernant la référence citée en objet.

FONDEMENT JURIDIQUE :
Cette réclamation est fondée sur {litigation.law}, qui prévoit une indemnisation forfaitaire ou un remboursement de plein droit en pareille circonstance.

DEMANDE :
Je sollicite le versement immédiat de la somme de {litigation.amount}.

À défaut de règlement ou de réponse satisfaisante sous un délai de 8 (huit) jours francs à compter de la réception de ce courriel, je transmettrai ce dossier au Médiateur compétent ainsi qu'aux autorités de régulation pour faire valoir mes droits.

Ce courriel a valeur de date certaine.

Dans l'attente de votre règlement,
Cordialement,

{user.name}
Généré et certifié par Justicio.fr
                    """
                    
                    print(f"💰 PAIEMENT VALIDE -> TENTATIVE ENVOI PRO VERS {target_email}")
                    success = send_stealth_litigation(creds, target_email, f"MISE EN DEMEURE - {litigation.company}", corps)
                    litigation.status = "Envoyé" if success else "Erreur"
                    db.session.commit()
        return "OK", 200
